...
class jspts:
    ...
    def parseOpts(self):
        parser = OptionParser(
            usage="%prog -s || -l logfile -p policyname [-v] (-h for more info + options)",
    ...
    def parseLine(self, polReg, line):
        ...
        if polReg.search(line):
            ...
            noDate = self.polSplitReg.split(
                line.strip(), 1
            )  # Strip Date and other eratta
            ...
            flowString = (
                ipData[4]
                + ","
                + self.convertProtoNum(elements[5])
                + "/"
                + ipData[6]
                + ","
                + elements[7]
    ...
    def printPolicy(self, options):
        if options.verbose:
            sys.stderr.write(
                "--> Generating specific Security Policy for all flows found ..."
            )
        ...
        for i in sorted(self.hostFlows.iterkeys()):
            for j in sorted(self.hostFlows[i]):
                ...
                if int(proto[1]) > 1024 and proto[0] != "ICMP":
                    sys.stdout.write(
                        "set applications application %s_%s protocol %s destination-port %s\n"
                        % (proto[0], proto[1], proto[0].lower(), proto[1])
                    )
                # Print a policy
                sys.stdout.write(
                    "set security policies from from-zone %s to-zone %s policy allow-%s-%s match source-address %s destination-address %s application %s_%s\n"
                    % (
                        polData[2],
                        polData[3],
                        proto[0],
                ...
                sys.stdout.write(
                    "set security policies from from-zone %s to-zone %s policy allow-%s-%s then permit\n"
                    % (polData[2], polData[3], proto[0], proto[1])
                )
                sys.stdout.write(
                    "set security policies from from-zone %s to-zone %s policy allow-%s-%s then log session-close\n"
                    % (polData[2], polData[3], proto[0], proto[1])
                )
    ...
    def main(self):
        ...
        if options.policyname == None:
            op.error(
                "You must specify a policy to summarise and file to parse (or STDIN)."
            )
        ...
        if options.read_from_stdin == True:
            ...
            sys.stderr.write(
                "!-> Ummm I need something to read to be useful. Please give me a -l or -s\n"
            )
            sys.exit(69)
